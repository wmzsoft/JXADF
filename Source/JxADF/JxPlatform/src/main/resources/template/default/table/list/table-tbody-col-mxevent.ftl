...
<#if (col.parameters.mxevent??)>
    ...
    <#elseif (col.parameters.mxevent=='ATTACHMENT')>
        ...
        <#t><img class='iconbutton' src='${baseSkin}/images/attachment${img!''}.png' <#rt>
                 ...
                 <#lt> upath='app.action?fromUid=${jbo.uidValue!0}&code=${col.parameters.mxevent_desc}&readonly=${(jbo.readonly)?string}&pagesize=10'/><#t>
    ...
    <#else>
        ...
        <#if (col.parameters.mxevent_icon??) >
            <#--当是图片的时候，按照图片处理-->
            <span class="toolbar_btn" onclick="${col.parameters.mxevent}(this,event,'${jbo.uidValue!0}')" mxevent="${col.parameters.mxevent}">
        ...
        <#else>
            ...
            <#else>                                        
                <#-- 当非图片的时候，按照文字处理，具体实现，看满足功能要求-->
                <input type="button" onclick="${col.parameters.mxevent}(this, event ,'${jbo.uidValue!0}')" class="btn_column"  <#lt>
                    value="<#t>
                <#if ((colDataValue?length) > ml) >
                    <#t>${colDataValue?substring(0,ml)}...<#t>
                <#else>
