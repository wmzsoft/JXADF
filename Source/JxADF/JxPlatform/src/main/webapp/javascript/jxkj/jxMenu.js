var JxMenu = (function(){
    ...
    JxMenu.prototype.createMenu = function (node, parent) {
        if (node != null) {
            ...
            for (var i = 0; i < node.length; i++) {
                if (!node[i].parent) {
                    var imgItem = $('<img/> ').addClass("moduleIcon").attr("src", this.setting.iconBase + "blank.gif");
                    var menuContainer = $('<div/> ').addClass("menuContainer")
                        .append($('<span/> ').addClass("triangleRight")).append(imgItem)
                        .append($('<div/> ').attr("id", node[i].app).addClass("menuItem").text(node[i].description).attr("appUrl", node[i].appUrl).attr("app", node[i].app).attr("appId", node[i].maxAppsId));
                    var topMenu = $("<div/>").addClass("topMenu");
                    topMenu.append(menuContainer).appendTo(menuDiv);
                    var url = this.setting.iconBase + "menu/" + node[i].app.toLowerCase() + ".png";
                    this.imgLoad(url, imgItem);
                    if (node[i].children != null) {
                        this.createMenu(node[i].children, $("<div/>").addClass("subMenu").appendTo(topMenu));
                    }
                }

                if (node[i].appType == "MODULE" && node[i].parent) {
                    isLastItemApp = false;
                    var menuItem = $("<dt/>").text(node[i].description).attr("appUrl", node[i].appUrl).attr("app", node[i].app).attr("appId", node[i].maxAppsId);
                ...
                if (node[i].appType == "APP" && parent) {
                    ...
                    if (!isLastItemApp) {
                        parent = $("<dd/>").append($("<em/>").text(node[i].description).attr("appUrl", node[i].appUrl).attr("app", node[i].app).attr("appId", node[i].maxAppsId)).appendTo(parent);
                        isLastItemApp = true;
                    } else
                        $("<em/>").text(node[i].description).attr("appUrl", node[i].appUrl).attr("app", node[i].app).attr("appId", node[i].maxAppsId).appendTo(parent);
                }
            }
        }
    };
    ...
    JxMenu.prototype.initData = function () {
        ...
        var self = this;
        this.menuDiv.find(".topMenu").hover(
            function () {
                self.loadModuleImg(this, "_hover");
            },
            function () {
                self.loadModuleImg(this, "");
            });
    };
